...
- Creating backups for sharded clustes is available straight from the UI. However, PMM does not handle restoring these backup artifacts end-to-end, and requires Percona Backup for MongoDB for this. For information on restoring sharded backups, check the [PBM documentation](https://docs.percona.com/percona-backup-mongodb/usage/restore.html).
