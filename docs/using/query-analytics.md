...
- The Filter panel on the left side of the dashboard lists filters grouped by category. It also shows the percentage of the main metric, which is explained below. If you select a different main metric, the percentages on the left panel will be related to this metric. Selecting one reduces the Overview list to those items matching the filter.
