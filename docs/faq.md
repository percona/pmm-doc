...
PMM Server embeds multiple components, like Victoria Metrics, Query Analytics, grafana, managed, postgresql, clickhouse, etc (components). All PMM Server component logs are managed and rotated with supervisord. Each particular component has its own log rotation config, which defineds both the maximum size of a log file and the number of log files to keep. The log rotation takes place once the log file size reaches its maximum size. 
