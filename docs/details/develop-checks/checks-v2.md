...
The check script assumes that a function with **check_context** accepts a `_list_` where each item represents the result of a single query specified in the check. Each result is a `_list_ of _docs_` containing returned rows for SQL databases and documents for MongoDB. It returns zero, one, or other check results that get converted to alerts.
