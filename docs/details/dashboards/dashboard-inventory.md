...
1. On the **Nodes** tab,  under the **Monitoring** column, click **OK** or **Failed** link for any specific node. A page that provides the user with crucial information regarding the total number of agents deployed on that node is displayed.
...
3. On the **Nodes** tab, under the **Options** column, click on the <image src="../../_images/dots-three-vertical.ico" width="15px" aria-label="triple dots" /> icon to check the properties and the current health status of an agent.
