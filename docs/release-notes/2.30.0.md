...
- WeNew real-time experimental dashboards for MongoDB are now available with more metrics and data like Hottest collections by Data Size, document count, and document read. These dashboards also provide Oplog details such as recovery Window, Processing Time, Buffer Capacity, and Oplog Operations.
