...
- New **Details** section groups together all the relevant parameters and labels for a Service. To keep things clean, this section is collapsed by default, so that you can view extra information only when you need it. 
...
- Services organized in clusters so that you can capture the bigger picture, instead of working at individual Service-level.
