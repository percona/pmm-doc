...
We've created a new experimental [MongoDB Router Summary](../details/dashboards/dashboard-mongodb-router-summar.md) dashboard to enhance your experience when monitoring MongoS (routers) of MongoDB sharded clusters.
...
Starting with PMM 2.43.0, we have officially ended support for CentOS 7, continuing the transition to Oracle Linux 9 that began with PMM 2.38.0. As part of this change, we will no longer release CentOS 7-based native packages (RPMs and DEBs) or EL7-based Docker images.

This shift allows us to streamline development and support efforts, focusing on modern operating systems that offer enhanced security, improved compatibility, and access to the latest libraries.

If your infrastructure still relies on CentOS 7, it’s crucial to plan your migration to a supported operating system before upgrading to PMM 2.43.0 or any future versions. Since PMM 2.38.0, all new Docker containers, OVF, and AMI installations have been based on Oracle Linux 9.

Our expert team is available to help you navigate the upgrade process to Oracle Linux 9, ensuring minimal disruption and optimal results. For more information or to explore our professional upgrade services, [contact our sales team](https://www.percona.com/about/contact).
