...
PMM 2.43 brings a host of updates, including redesigned dashboards, new collectors and metrics, enhanced troubleshooting capabilities, strengthened security, and expanded platform support with experimental ARM compatibility for PMM Client.
...
### Monitoring for Percona Backup for MongoDB (PBM)

PMM now includes support for monitoring your PBM setup, providing valuable insights with metrics related to `pbm-agent` statuses, PITR (Point-In-Time Recovery) configuration, and backup statuses.
...
- `mongodb_pbm_cluster_backup_configured`: Indicates whether PBM is configured for this cluster. Returns 1 if PBM is configured, 0 if not.

- `mongodb_pbm_agent_status`: Describes the status of each PBM agent connected to the nodes in the cluster. Returns 0 for agents that are functioning correctly, and ≥1 for agents that are down (e.g., 2 for currently unreachable agents).

- `mongodb_pbm_cluster_pitr_backup_enabled`: Specifies if PITR is enabled in the current PBM configuration. Returns 1 if PITR is enabled, 0 if not.

- `mongodb_pbm_backup_size`: Indicates the size of each backup in PBM's backup history. Includes three labels for differentiation: `opid` (backup operation ID), `status` (backup status), and `name` (backup name).

- `mongodb_pbm_backup_duration_seconds`: Shows the duration of each PBM backup operation. Uses the same labels as `mongodb_pbm_backup_size`.
...
#### Viewing and using PBM metrics
Currently, you can view these metrics in the **Explore** tab of PMM. To access them:

1. On the side menu in PMM, choose **Explore > Metrics**.
2. In the **Metrics** field, enter **mongodb_pbm** to see all PBM-related metrics.
3. Click **Run query** to view the results.

This is just the start! While you can create your own dashboards using these metrics, we're working on dedicated PBM dashboards to provide an at-a-glance view of your backup status. We're also developing alert templates to help you proactively monitor your PBM setup.
...
As an experimental feature, ARM support will be refined and stabilized in future releases, paving the way for unified monitoring across x86 and ARM architectures. Test this feature and share your experience on the [PMM forum](https://forums.percona.com/c/percona-monitoring-and-management-pmm/percona-monitoring-and-management-pmm-v2/31) to help accelerate its path to GA.
...
Need help with the upgrade process? Our team of PostgreSQL experts can guide you through a smooth and efficient upgrade. [Contact our sales team](https://www.percona.com/about/contact) to learn more about our professional upgrade services and ensure a seamless transition to a supported PostgreSQL version.
