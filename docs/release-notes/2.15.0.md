...
_Disable collectors during adding node/service to monitoring_
    ...
    With this feature, users can disable any collector PMM uses to get metrics. In the situation where metrics could not be collected or are not needed at this point, disabling the collector(s) prevents PMM from flooding logs or saves infrastructure resources if the metrics simply aren’t needed.
Note: our vision for PMM collectors is to provide “stop from collecting” functionality to prevent possible harm PMM can do to the user environment. Providing the above “disable” feature is an initial step toward the ideal functionality. The full and flexible management for “What metrics to collect and in what resolution” is slated for future releases. 


_External services monitoring_

    Starting in PMM v1.4.0, users were provided with the ability to monitor external services Percona didn’t currently support (e.g., Redis). This blog article from 2018 nicely described external services monitoring at that time. Note: at the time this blog was written in 2018, Percona was not natively supporting a PostgreSQL monitoring service and so his was listed as an external service. Today, PostgreSQL is natively supported by PMM.

Until now, PMM v2.x didn’t unfortunately support external services monitoring. With this release, however, any non-natively supported by PMM   service will now become supported with external services monitoring. You can see the list of possible exporters to be used in https://prometheus.io/docs/instrumenting/exporters/.  Of course, natively supported services will still deliver an expanded set of metrics and insights but some display is better than none!

_Provide summary information for  systems  (pt-*-summary actions)_

    Like most things in life, you start high-level and then drill into the details. With the addition of “pt-*-summary” in PMM 2, users can now view summary information about services and nodes on PMM’s dashboard. This summary information is in the industry common format of pt-*-summary tools output to simplify portability of this data. This format will also be preserved in the snapshot of the dashboard shared with Percona Support to simplify investigations of issues.
...
- HAProxy support by PMM
    ...
    Users are able to add HAProxy Services for the monitoring in PMM2. The support level of them in PMM will be the same we have for ProxySQL  - so they will be presented in Inventory and on Dashboard.  
This will allow users who use HAProxy in their HA configuration to have this component also monitored by PMM.
PMM in future releases will start use HAProsy as default for the DBaaS feature and also will use this functionality to monitor HAProxy.


- DBaaS Preview improvements (Technical  Preview)

    From now the user will be able to see the progress over internal steps the system needs to do to execute some operations with DBaaS. 
Progress Bar will not be time-related and present only steps.
Progress Bar component will also reflect the K8s/Operator-related errors to the user, so in case of some errors, the user will have the error text on UI, no need to use K8s tools to see the error. 
With the same UI, users will be able to see the latest logs from K8s so they will have even more information about why the error happened.  
Known Limitations: 
The progress bar will not provide valuable information for the Delete operation (will be in a later version when we’ll change the API with Operators Team), 
Operation of DB Cluster Modification will have “strange” behavior and will star changes from non-zero values of steps. (Will be modified after API changes)
