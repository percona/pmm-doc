...
- For precision, we have added a unit (%) to the CPU Anomalies panel.
...
- Enabled sparklines on the Disk Read and Write panels to visually represent and show trends in the data.
 


### Enhanced main menu

- With the new navigation structure in PMM 2.32.0, you have easy access to all dashboards for monitored services. Dashboards in the services folder that were not accessible from the main menu are now accessible from "Other Dashboards."

For example, to access the dashboards for MYSQL, go to *MYSQL >> Other dashboards*.

- Only monitored services are now included in the main menu, which enhances the user experience and makes the UI more intuitive.
...
- [PMM-9946](https://jira.percona.com/browse/PMM-9946): For a simplified DBaaS experience, when you deploy PMM inside the K8s cluster using, PMM automatically adds this cluster for monitoring, thus forgoing the need to add the DB cluster manually.
