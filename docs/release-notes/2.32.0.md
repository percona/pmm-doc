...
In specific environments, including AWS, some EPEL repository mirrors did not respond within the time limit defined by `pmm-update,` currently set to 30 seconds. It was causing `supervisord `to kill `pmm-update-checker`, the component responsible for determining if a newer PMM Server is available for upgrade. This issue has been resolved now.
