...
2. Create a database cluster for each database cluster you have created in PMM. You can use the [script](https://github.com/percona/pmm/blob/main/migrate-dbaas.py) to do it automatically during the maintenance window. Keep in mind that the migration script uses the default configuration to connect to the Kubernetes cluster and run migration against it.
