...
As part of PMM 2.38.0, you can now see the number of agents running on any particular node. When you click on any node, the UI navigates to the view of agents, which is filtered to display only agents related to that specific node. You can also view comprehensive information about each agent and check their health status to see if they are running or have failed.
