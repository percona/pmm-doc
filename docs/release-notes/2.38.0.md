...
After migration:
    ...
    ORDER BY t
    ```    
As you can see, Altinity datasource used to operate with a number of macros, like `$timeSeries`, `$timeFilter` and `$table`, which are no longer present in the datasource from Grafana. This is why the query has to be re-written in a way to no longer rely on old macros, but to leverage instead the ones offered by the new plugin. You will notice that in the example above, the macro `$timeFilter` has changed to `$__timeFilter` and now it requires a parameter, which should be a column of type datetime.

Indeed, you migration effort will vary depending on which macros you used, but in general we encourage you to consult the plugin [documentation](https://grafana.com/grafana/plugins/grafana-clickhouse-datasource/) to properly apply the new syntax, including macros and template variables.
