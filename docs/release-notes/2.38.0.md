...
### New default Alert template

We've added a new default alert template which you can use [Percona Alerting](../get-started/alerting.md) to monitor your Node status and inform you if the Node goes down.

This new alert template can be used with all other alert templates and consumes the same notification channels.

Prerequisites: Before using this template, make sure that:

 - the PMM Agent is healthy and present on the Database Node.
-  PMM Client is be on version 2.38 version or later.
