...
As security is paramount to us, starting with PMM 2.38.0, we have laid a solid and modernized enterprise-grade foundation for PMM. We have migrated our base operating system on which we build our PMM image from CentOS 7 to Oracle Linux 9. Thus starting with PMM 2.38.0, we have made PMM publicly available on Oracle Linux 9.

There are many benefits of this transition. The migration fixes a large number of CVEs and will see improved upstream response time to security-related issues. It also uses more modern libraries for our team to build on along with better compatibility with new technologies. 

We will be rolling this out in phases as not all users will be able to seamlessly make the move. 
...
*Users with a bit longer wait time*
