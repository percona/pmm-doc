...
- **Integrated Alerting (Technical preview):**
    - Alerting in PMM now has an additional notification channel type - webhooks. So now, users can integrate Alerting with any tool they use for Incident management.  Read more about new notification channels and how to set up them in [our documentation  ](https://deploy-preview-626--pmm-doc.netlify.app/using/alerting.html#add-a-notification-channel)
