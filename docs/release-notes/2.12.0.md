...
The Push/Pull mode is controlled by "--metrics-mode" flag for "pmm-admin config" and "pmm-admin add" commands. 
For 2.12 the default value will be for the 'pull' mode. 
You can set it to push by this flag during the adding and configuring. 
IF you need to change metrics mode for existed Service you need to remove it and add the same name and needed flags. There is no "update" functionality as of now.

In later releases, we'll switch to use the push model by default for the newly added Services  
