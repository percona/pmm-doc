...
Unprotected passwords and credentials are among the top ten security risks identified by OWASP (Open Web Application Security Project). Previously, while PMM required monitoring accounts with minimal permissions, a compromised account could still expose sensitive data.
...
PMM now provides full monitoring support for its internal PostgreSQL databases, with metrics displayed across Query Analytics (QAN).
