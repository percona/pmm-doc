...
To better support longer MongoDB queries, especially those involving aggregation pipelines, we've increased the default maximum query length limit in Query Analytics (QAN)  from 2048 to 4096 characters.

This adjustment reduces parsing issues and minimizes occurrences of the *"Example query is not successfully parsed due to the max query length limit"* message on the QAN page.

## Added monitoring support for default PostgreSQL database

PMM now collects and displays metrics from the PMM's internal PostgreSQL database. While using the default database is not a recommended practice, this update ensures more complete monitoring coverage for users who rely on it.
The change enables metrics collection from the default database across all PostgreSQL-related dashboards and QAN providing more comprehensive performance insights.
