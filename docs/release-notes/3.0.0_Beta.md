...
You can now run database exporters from any PMM Client node, which means you can distribute the monitoring load across multiple PMM Agents. This is especially useful for monitoring cloud-hosted databases like RDS.

When adding a new service through the PMM UI, you can now select the node that will run the exporter. By default, PMM Server is chosen, but you have the option to select any node with a PMM Agent installed. If a node has multiple agents, you can specify which Agent to use.
...
For more details, refer to our updated documentation on adding services through the UI.
