...
Unprotected passwords and credentials are among the top ten security risks identified by OWASP (Open Web Application Security Project). Prviously, while PMM required monitoring accounts with minimal permissions, a compromised account could still expose sensitive data.
...
The transition to service accounts brings fine-grained access control and logging of all actions performed, providing more security, better visibility and auditing.
...
We've added automated update support for AMI/OVF deployments. The new **Updates** page also enables AMI and OVF deployments to update PMM Server directly from the UI, following the integration of the Watchtower container.
...
PMM 3 introduces a major overhaul of the upgrade system, replacing the previous method that relied on internal package updates. While the previous approach was convenient, allowing users to upgrade directly via the **Update** button on the Home dashboard, it was susceptible to connection issues and package corruption that could lead to complex recovery situations.

The new upgrade method movies away from RPM updates to exclusive container updates, eliminating RPM-related complications. This streamlined approach provides consistent upgrade experiences across all deployment types—Docker, Podman, and OVF/AMI.
...
PMM streamlines the database monitoring setup process by shifting from CLI-based configuration to an intuitive UI-driven approach. After installing PMM Client on your database server, all further monitoring configurations can be managed directly through the PMM interface, eliminating the need for complex command-line operations.

This simplified workflow combines with PMM's new ability to run database exporters on any PMM Client node rather than solely on PMM Server. This distributed approach enables monitoring of a larger number of remote and cloud-hosted databases like RDS instances by spreading the monitoring load across multiple PMM Clients.
...
We've simplified AWS installations to match our standard Docker/Postman workflow. After installation, you'll immediately see the PMM login screen.
...
PMM now provides full monitoring support for its internal PostgreSQL databases, with metrics displayed across all PostgreSQL dashboards and Query Analytics (QAN).

This enhancement resolves a previous visibility gap where some database activity was hidden when applications used the default database.
...
The latest version of MongoDB, along with its Percona version (link link?), brings [numerous improvements and significant performance enhancements](https://www.mongodb.com/docs/manual/release-notes/8.0/).
