...
!!! caution alert alert-warning "Disclaimer"
    Deploying PMM on Kubernetes is still under Preview and is subject to change. We recommend that early adopters use this feature for testing purposes only.

Starting PMM 2.29.0, we have introduced the [helm](https://helm.sh/) chart supported by Percona to seamlessly deploy your PMM instances on the k8s, a prominent container orchestration system. With the Helm chart can customize your deployment. You can tweak your PMM installation by setting these various [parameters](https://github.com/percona/percona-helm-charts/tree/main/charts/pmm#parameters). 
...
!!! caution alert alert-warning "Disclaimer"
    Podman support is still under Preview and is subject to change. We recommend that early adopters use this feature for testing purposes only.
...
Created a new experimental dashboard for PXC Cluster Summary to make it simple, intuitive, and provide at-a-glance visibility for better decision making. Critical data for the PXC cluster is put together in this dashboard to obtain feedback from our users.
...
As a PMM user, you can now explicitly set a detailed level of logging so that, by default, you will have meaningful logs to see, thereby enhancing your troubleshooting experience. So if you want detailed logging, you can set `--log-level` to INFO. By default, INFO is not logged. Thus, you have more control over the logs you want to view with the --log-level parameter with the flexibility to choose options such as warning, error, fatal, and info.

#### Additional parameters added in commands

- Added the agent (exporter) port to the output of the `pmm-admin` commands which is extremely crucial during debugging.
...
Replaced exporters for pmm compatibility:
...
- [PMM-9981](https://jira.percona.com/browse/PMM-9981): Fixed excessive space usage for Group replication monitoring. Services with enabled group replication and custom queries now take up less disk space and perform faster. To achieve this, we have removed the **Transactions Details** table from the **Group Replication** dashboard, as these were based on labels that were generating high-cardinality data.
