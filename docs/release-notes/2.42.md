...
You can now configure the maximum number of connections that the PostgreSQL exporter can open to the same PostgreSQL instance.
...
## Configure maximum connections for pmm-agent
pmm-agent can open connections to monitored databases to perform queries for needs of QAN, Advisors and other systems. Before there was no limit on amount of simultaneously opened connections to single DB instance which could cause resource utilization spikes. As a part on performance optimization initiative we added limintations on that. Now pmm-agent tracks all needed resources (connections mainly) and opens no more than two connections per instance at once. Default value can be changed with parameter or env variable on pmm-agent startup
