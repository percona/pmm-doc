...
!!! caution alert alert-warning "Important"
    PMM upgrade fails for versions prior to 2.33.0. Even though we have a [workaround](2.33.0.md#known-issues), we highly recommend using the latest version of PMM. This ensures that you have access to the latest PMM features and that your environment runs on the latest version of the underlying components with all the bug fixes in place.


## Release Highlights


### Data loss prevention solution for PMM server outages

Starting with PMM 2.33.0, we have introduced a new solution to prevent data loss with PMM. In case of a network outage between PMM server and PMM client, metrics are not collected. This causes a loss of historical data, which can be important when analyzing trends, diagnosing problems, or investigating an issue from an earlier period. 
...
- [PMM-10135](https://jira.percona.com/browse/PMM-10135): Starting with PMM 2.33.0, for MySQL, the *Explain* tab is supported without the *Examples* tab. If a query in the *Explain* tab contains sensitive data, placeholders will replace them.
