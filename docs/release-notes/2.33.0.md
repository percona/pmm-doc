...
Starting with PMM 2.33.0, we have introduced a new solution to prevent data loss with PMM. In case of a network outage between pmm-managed and pmm-agent, metrics are not collected. This causes a loss of historical data, which can be important when analyzing trends, diagnosing problems, or investigating an issue from an earlier period. 

As a solution to the problem mentioned above, if the PMM Server goes down, metrics will be buffered on the monitored instance until PMM Server returns online, which is when metrics will get pushed to PMM Server. This only applies to PMM Clients which run in the `push mode`. The same holds true when a brief network outage occurs on the client side.
...
- [PMM-11126](https://jira.percona.com/browse/PMM-11126): A bug in the PMM Server ansible scripts caused PMM to upgrade Nginx's dependencies without updating Nginx itself. Due to this, PMM throws an error while upgrading and cannot upgrade to a newer version. The issue has been resolved now.
