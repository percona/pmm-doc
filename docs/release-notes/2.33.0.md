...
Stage 2: HA data sources

As part of PMM HA stage two in HA we will implement the following:

Configure Grafana Data sources
Migrate grafana rom using from SQLite to PostgreSQL
Configure pmm-clients to push metrics to Victoria metrics
I want to have the ability to use an external PostgreSQL Server as a data storage for PMM. So all pmm-managed data is stored there.
Remove dependency from /srv volume
Base files (prometheus, alertmanager, CRDs)
All base files are stored in DB.
prometheus.base.yml
alertmanager.base.yml
DBaaS Base CRD files
maybe other files
Use base files stored in DB by pmm-managed features
Content of existing base files is migrated to DB
...
Config file for PMM
Ability to turn off internal DBs (Configure Supervisord)
Migration plan for moving to distributed system

Stage 3: Clusterized PMM

Communication between PMMs - selection of the Main PMM
PMM in passive mode
Disable pmm-agent
Disable firing alerts
Disable running cron jobs (backup) - don’t show error
