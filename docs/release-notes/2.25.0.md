...
** From this release, PMM will let users who add PostgreSQL Servers be monitored by PMM to specify database name for monitoring. Previous versions have the default `postgres` database, which wasn't always convenient for the users. [Link to documantation](TBD)
