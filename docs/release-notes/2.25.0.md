...
- **Enhanced PostgreSQL monitoring**
    - From this release, PMM will let users who add PostgreSQL Servers be monitored by PMM to [specify database name for monitoring](https://deploy-preview-643--pmm-doc.netlify.app/details/commands/pmm-admin.html#postgresql). Previous versions have the default `postgres` database, which wasn't always convenient for the users. 
    - This Release of PMM is supported and compatible with the recent release of the PostgreSQL extension - `pg_stat_monitor` v1.0.0. You can read more about its features and advantages in this blog post (LINK to pg_stat_monitor release announcement or Userguide)
