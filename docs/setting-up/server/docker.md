...
| `DATA_RETENTION`                                                | The number of days to keep time-series data. <br />**N.B.** This must be set in a format supported by `time.ParseDuration` and represent complete number of days. The supported units are `ns`, `us` (or `µs`), `ms`, `s`, `m`, and `h`. <br /> So the value must be a multiple of 24, e.g., for 90 days 2160h (90 * 24).
