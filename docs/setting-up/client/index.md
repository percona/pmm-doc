...
> **Note** Clients *must* be registered with the PMM Server using a secure channel.  If you use http as your server URL we will try to connect via https on port 443 and if we can't establish TLS you will get an error and must use https along with the appropriate secure port. 
