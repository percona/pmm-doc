...
* :pmmbug:`5064` and :pmmbug:`5065`: Alert manager available in PMM 2 since this
  release allows to supply dashboard panels with the rules, which can be
  evaluated with a specified periodicity to send pre-configured notifications
  in case if the specified conditions are met.
* :pmmbug:`5097`: PMM Client docker image is now available for installation in
  addition to previously available DEB and RPM packages and a tarball.
