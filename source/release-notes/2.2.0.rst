...
* :pmmbug:`4950` and :pmmbug:`3094`: `AWS partitions <https://docs.aws.amazon.com/sdk-for-go/api/aws/endpoints/#pkg-constants>`_ are now supported when adding an 
  Amazon RDS instance to PMM; both default ``aws`` one and alternative
  partitions are scanned automatically during the discovery process.
